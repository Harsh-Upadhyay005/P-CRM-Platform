    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="h-28 rounded-xl border border-white/5 bg-slate-800/30 animate-pulse"
          />
        ))}
      </div>
    );
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
      {stats.map((stat, i) => {
        ...
        return (
          ...
          >
            <div
              className={`absolute -top-8 -right-8 w-24 h-24 rounded-full blur-2xl opacity-30 ${c.bg}`}
            />
